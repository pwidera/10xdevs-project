...
import { AIGenerationService, createAIGenerationService, OpenRouterError } from "../ai-generation.service";
  ...
  describe("generateFlashcards()", () => {
      ...
      it("computes consistent hash for same source text", async () => {
        const mockResponse = {
          ...
          choices: [
            {
              message: {
                content: JSON.stringify([{ front_text: "Q", back_text: "A" }]),
              },
            },
          ],
        };
        ...
        mockFetch.mockResolvedValueOnce({
          ...
          json: async () => ({
            ...
            choices: [
              {
                message: {
                  content: JSON.stringify([{ front_text: `  ${longText}  `, back_text: `  ${longText}  ` }]),
                },
              },
            ],
          }),
    ...
    describe("error handling", () => {
      ...
      it("throws OpenRouterError when response has no choices", async () => {
        ...
        await expect(service.generateFlashcards(validParams)).rejects.toThrow("No response from AI model");
      });
      ...
      it("throws OpenRouterError when response content is not valid JSON", async () => {
        ...
        await expect(service.generateFlashcards(validParams)).rejects.toThrow("Failed to parse AI response as JSON");
      });
      ...
      it("throws OpenRouterError when response is not an array", async () => {
        ...
        await expect(service.generateFlashcards(validParams)).rejects.toThrow("AI response is not an array");
      });
      ...
      it("throws OpenRouterError when no valid proposals generated", async () => {
        mockFetch.mockResolvedValueOnce({
          ...
          json: async () => ({
            ...
            choices: [
              {
                message: {
                  content: JSON.stringify([{ front_text: "", back_text: "" }, { invalid: "proposal" }]),
                },
              },
            ],
          }),
        ...
        await expect(service.generateFlashcards(validParams)).rejects.toThrow("No valid proposals generated");
      });
