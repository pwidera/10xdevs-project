        ...
        {sent ? (
          ...
          <form onSubmit={onSubmit} className="space-y-4" noValidate>
            ...
            <div className="space-y-2">
              ...
              <Input id="email" name="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} onBlur={validate} disabled={loading} required aria-invalid={!!error} aria-describedby={error ? 'email-error' : undefined} />
              {error && <p id="email-error" className="text-xs text-red-600">{error}</p>}
            </div>
            <Button type="submit" disabled={loading} className="w-full">{loading ? 'Wysyłanie…' : 'Wyślij instrukcję'}</Button>
          </form>
        )}
      </CardContent>
    </Card>
